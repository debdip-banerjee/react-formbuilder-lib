...
export type IField = ITextField | INumberField | IDateField | IDropdownField | ISegment | ISwitch | IEmailField | IPasswordField | IRadioField | IAutocompleteField | ITextarea | ICheckbox | IRepeatableSection | IFileUploader | ICustomWidget;
export type LoadingParams = {
    [key: string]: boolean;
};
export type LastChangedObjectProps = {
    obj: any;
    fieldname: string;
    parent?: string;
};
export type IUncategorisedFiles = {
    files: any[];
    meta?: {
        [key: string]: any;
    };
};
export type ICategorisedFiles = {
    [key: string]: IUncategorisedFiles;
};
export type IFileExtensions = "image/*" | ".pdf";
export type IAlertTypes = 'success' | 'error' | 'warning' | 'info';
export type IMessageAlert = {
    type: IAlertTypes;
    message: string;
};
export type IRflError = {
    isBlocking?: boolean;
    type?: IAlertTypes;
    message: string;
};
export type IFieldDataEmitter = {
    field: IField;
    fieldname: string;
